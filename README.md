...
> The code in `egittins/`, `experiments/` and `tests/` was written by me with AI assistance
> (Claude Code). RESULTS.md and LABELFREE_EXPLAINER.md were prepared with AI assistance from the
> CSVs in `results/`; each has a note at the top saying where its numbers come from.

Simulation of **empirical Gittins** scheduling in the M/G/1 queue, policy discussed in:
...
```bash
pip install -r requirements.txt            # torch needed for imitation and (failed :( ) RL experiments
python -m pytest tests -q                 # tests: direct kernel, simulator vs. P-K and E[S]/(1-ρ)
...
You can consult `RESULTS.md` for every number (with CIs) & where to produce it.
...
| Concept (paper) | Code |
|---|---|
| Job size distributions on a grid, empirical distribution, truncation, the 1-6-14 and bounded Pareto distributions | `egittins/distributions.py` |
| Gittins rank function of a finite-support distribution for every age on the grid; O(K+L) convex-hull kernel with the O(KL) definition kept as a reference | `egittins/gittins.py` |
| Preemptive M/G/1 under any SOAP policy: min-rank service, FCFS tie-break, PLCFS fallback at rank ∞; FCFS and PLCFS baselines | `egittins/simulate.py` |
| Section 7 protocol: 100 trials × (distribution, ρ, n), empirical vs. truncated empirical Gittins vs. true Gittins, FCFS, PLCFS | `experiments/baseline_comparison.py` |
| §8.5 "gray-box scheduler": refit empirical Gittins each busy period from the last *K* completed jobs, under distribution drift | `experiments/drift_window.py` |
...
| k-updating stream engine: oracle / FCFS / static fit / k-updating with several windows on one common-random-number stream, paired ratios with t CIs | `egittins/kupdating.py`, `experiments/kupdating_drift.py` |
...
| (empirical vs. true rank functions and tails and their ratio) | `experiments/eda_tails.py` |

### Modelling choices

- **Grid step h = 0.01.** All job sizes are multiples of h, so every distribution is
  (secretly) discrete and the Gittins rank is computed exactly rather than approximated. This is
...
- `headline`: unidirectional weight drift (0.1, 0.3, 0.6) → (0.6, 0.3, 0.1) on 1-6-14 (variability
...
- `sweep`: mean-preserving weight drift (load fixed) as a triangle wave (linear/piecewise/periodic) with period
...
figures/        generated PNGs, named sNN_* by section
