"""
drift.py — non-stationary job size distributions for the "learning while
scheduling" experiments (Section 8.5 of the paper).

A `DriftModel` maps the index k of a *measured* busy period to the true job
size distribution F_k in force during that busy period. It is the composition
of a schedule u(k) ∈ [0, 1] with a one-parameter family of distributions
D(u); D is evaluated on a grid of `n_grid` values of u and cached, so the
genie policy γ(F_k) is computed once per grid point.

Schedules:
  * `triangle(T)`  — u goes 0 → 1 → 0 with period T busy periods (recurring drift)
  * `ramp(N)`      — u goes 0 → 1 linearly over N busy periods (one-way drift)
  * `constant(u)`  — stationary control

Families (all on the paper's grid h = 0.01):
  * `mean_preserving_weights` — the 1-6-14 mixture with its component weights
    moved along the line where E[S] is fixed (mass flows between the 6-mode
...
Load convention (as in `experiments/drift_window.py`): the load ρ is held
fixed and the arrival rate is λ_k = ρ / E[F_k]. For the mean-preserving family
λ is therefore also (essentially) constant.
"""
