"""
imitation.py — a rank function learned by imitating empirical Gittins.

Empirical Gittins maps (age a, sample window) → rank through an exact but
opaque computation. Here an MLP learns that map from *features* of the
conditional excess sample (S − a | S > a), so that the same network can be
dropped into the simulator as a SOAP policy for any sample window, and so
that we can ask which features carry the information. The point of the
exercise is the ablation: without the two *hazard* features (gap to the next
sample atom above a, and the conditional mass at it) the network cannot see
the "a jump is coming, finish the job" structure of the rank function.

Features at age a, from the sorted sample s (units), with c = #{s > a} and
mean excess m = mean(s − a | s > a), in three nested sets (FEATURE_SETS):
    tail_only:  log Ḡ(a), log c, log m
    no_hazard:  + log(q_p / m) for p ∈ {0.1, 0.25, 0.5, 0.75, 0.9}, log(max excess / m)
    full:       + hazard:  log(gap / m),  log(mass at next atom / Ḡ(a))
Target: log(r(a) / m), r the exact empirical Gittins rank (r ≤ m always).
Scale enters only through log m (in units), so the map is otherwise scale-free.

Training distributions are random grid distributions (Gaussian mixtures,
bounded Pareto, lognormal, Weibull, few-atom discrete), with the paper's
1-6-14 and Pareto(2, 1.2, 500) parameters excluded, and random sample sizes.
"""
...
# Feature ladder for the ablation: what does the network need to see?
FEATURE_SETS = {
