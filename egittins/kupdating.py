"""
kupdating.py — the "gray-box" scheduler of Section 8.5 under distribution drift.

A stream of busy periods k = 0, 1, ... is simulated with true distribution
F_k = drift.dist(k − n_warm). Every policy sees the *same* arrival/size stream
in every busy period (the busy period's seed is shared, and the arrival process
does not depend on the scheduler), so all comparisons are paired.

Policies:
  * genie        — true Gittins γ(F_k) for the current F_k (the oracle)
  * fcfs
  * static       — empirical Gittins fitted once, at the first measured busy
                   period, from the last `static_window` completed sizes
  * kupd_w       — k-updating: refit empirical Gittins at the start of every
                   busy period from the last w completed sizes, for each w in
                   `windows`

Refits happen only at busy-period boundaries: within a busy period the set of
completed jobs is policy-dependent (Gittins finishes short jobs first) and so
is a biased sample; at a boundary every arrival has completed.

`run_stream` returns per-busy-period sums of response times for every policy,
so that mean response times, paired ratios, and time courses can be formed
afterwards. `summarize` turns a list of trial results into mean ratios vs the
genie with 95% t confidence intervals.
"""
