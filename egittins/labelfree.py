"""
labelfree.py: a rank function found by derivative-free search on simulated response time.

The training signal is the simulated mean response time; Gittins ranks are not used in
training. A candidate rank function is tabulated over integer ages, run in the SOAP
simulator and scored. CMA-ES proposes the next generation. Every candidate in a generation
is scored on the same simulation seeds (common random numbers), and each generation draws
fresh seeds. Only the ordering of ranks across ages affects the policy, so learned ranks are
compared with Gittins by ordering (Spearman correlation, monotone rescale).

Policy classes:
  * knot_rank      Stage 0: log-rank at K knots equally spaced over age, linear interpolation.
  * feature_rank   Stage 1: a linear map or a one-hidden-layer (SiLU) net from the
                   standardized feature tiers of `imitation.FEATURE_SETS` to a rank,
                   tabulated over integer ages from a sample window as `nn_policy` does.
                   Trained across several random distributions with loss = mean over
                   distributions of MRT / MRT(FCFS) on the same seed.
"""
...
def policy_from_rank(rank: np.ndarray, name: str) -> Policy:
    """Vectorized version of `gittins.policy_from_rank` with identical output: next_atom[a] is
...
def knot_rank(theta: np.ndarray, L: int) -> np.ndarray:
    """Rank over ages [0, L): exp of the linear interpolation of log-rank knots `theta`
    placed at equally spaced ages 0, ..., L-1."""
