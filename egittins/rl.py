...
The policy class is the same as for imitation: a rank table r(a) over integer
ages produced by `RankNet` from features of a fixed sample window. Training
makes it stochastic: at every decision point (busy-period start, arrival,
completion) the job to serve is drawn with probability ∝ exp(−r(age)/τ) and
runs until the next event. An episode is one busy period; its return is
−(area under the number-in-system curve) = −(sum of response times), and the
return-to-go from a decision is minus the area from that decision onward.
REINFORCE with batch-normalized advantages, Adam.

Two arms: from scratch (random network) and fine-tuning the imitation net.
Evaluation is always deterministic (τ → 0) in the standard SOAP simulator.
"""
