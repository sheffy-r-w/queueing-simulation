...
Model (Section 2 of the paper): Poisson arrivals at rate λ = ρ / E[S], i.i.d.
job sizes from a true distribution F, one server, fully preemptive, no
overhead. The scheduler is a SOAP policy: it always serves the job of minimal
rank, where rank is a function of the job's age (attained service). Ties at a
finite rank are broken FCFS; ties at rank ∞ are broken preemptive-LCFS.

Time discretization: everything is in quanta of length h (the grid step of
the job size distributions). Inter-arrival times are rounded to the nearest
quantum (minimum 1), which keeps the mean inter-arrival time, and hence the
load, unbiased.

Why this is fast enough: on each interval between atoms of the policy's
distribution the Gittins rank is decreasing in age (Observation A.1), so the
job in service cannot be overtaken by a waiting job except at an arrival or
when it reaches its next atom. The simulator therefore only makes scheduling
decisions at arrivals, completions, and atom crossings ("skip-ahead").
For FCFS / PLCFS there are no atoms, so decisions happen only at arrivals and
completions.

The core loop is compiled with numba.
"""
...
def _simulate_kernel_skip(rank, next_atom, lam, h, atoms_u, cdf, n_busy, seed,
                          max_total, cap):
    """Same schedule as `_simulate_kernel`, bit-identical per-job response times, fewer decisions.

    The reference kernel re-selects the minimum-rank job (an O(m) scan) every time the served
    job crosses an atom of the policy — for a dense true distribution, every grid point. The
    served job can only lose the server at those same points, and only to the best *waiting*
    job, whose rank is fixed until the next arrival or completion. So after one selection this
    kernel walks the served job's chain of atoms with an O(1) "does it still win?" test (rank,
    then the finite-FCFS / infinite-PLCFS tie-break on sequence number, exactly as the scan
    would decide) and only stops where the reference kernel would have switched jobs.
    """
    np.random.seed(seed)
    L = rank.shape[0]
    K = atoms_u.shape[0]
...
def simulate(policy: Policy, F: GridDistribution, rho: float, n_busy: int,
    ...
    kernel = "skip" (default): `_simulate_kernel_skip`, which re-selects only where the served
    job can actually lose the server; "reference": `_simulate_kernel`, the original. The two
    give bit-identical per-job response times (tests/test_simulator.py)."""
    if not (0 < rho < 1):
