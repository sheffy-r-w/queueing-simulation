"""
Learning while scheduling under distribution drift (Section 8.5 of the paper).

The paper's one-shot analysis fixes the job size distribution. Section 8.5
proposes a "gray-box" scheduler for the non-stationary case:

    at the start of each busy period, construct an empirical Gittins policy
    from the sizes of the most recent P completed jobs, and run it for the
    duration of that busy period,

and asks how fast the distribution can drift while this remains near-optimal.
P is the window size: a hyperparameter trading *variance* (small P → noisy
empirical distribution) against *bias* (large P → stale, pre-drift data).

Why "at the start of a busy period": within a busy period, completion order is
policy-dependent (short jobs finish first under Gittins), so the set of
completed jobs is a biased sample. At a busy-period boundary every job that
arrived has completed, so the history is an unbiased record of the arrivals.

Drift model. The true distribution at busy period k is a mixture interpolation
F_k = (1-θ_k) F_A + θ_k F_B on the common grid, with θ_k a triangle wave of
period T busy periods (0 → 1 → 0). T = ∞ is the stationary case. F_A is the
paper's 1-6-14 distribution; F_B is a 3-8-14 mixture (a new workload type whose
share of the traffic is θ).

Measured quantity: mean response time over the measurement horizon, divided
by that of an *oracle* that uses true Gittins γ(F_k) for the current F_k.
All policies see the same arrival/size stream (common random numbers).

Run:  python -m experiments.drift_window --quick     # smoke test, ~1 min
...
def make_endpoints(h=0.01):
    ...
    The interpolation F_k = (1-θ)F_A + θF_B is "a fraction θ of the traffic now comes
    from a new workload type". The small mode moving from 1 to 3 is what hurts a stale
    policy most: Gittins fitted on old data gives up on a job at age ≈1.5, exactly
    where the new small jobs still need service.
    """
