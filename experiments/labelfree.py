"""
Section 13 (label-free): a rank function found by CMA-ES on simulated mean response time.
Gittins ranks are not used in training.

  stage0   control on 1-6-14, rho = 0.8: log-rank at 32 knots over age, CMA-ES with common
           random numbers, 3 optimizer seeds; paired evaluation on fresh seeds against true
           Gittins and FCFS.
           -> results/s13_labelfree_stage0*.csv, figures/s13_labelfree_stage0.png
  train    Stage 1: for each feature tier in egittins.imitation.FEATURE_SETS, a linear (or
           one-hidden-layer) map from the tier's features to a rank, trained across N random
           distributions (1-6-14 and Pareto(2, 1.2, 500) held out); loss = mean over
           distributions of MRT / MRT(FCFS) on the same seed.
           -> results/s13_labelfree_train*.csv, results/s13_labelfree_params.npz
  static   held-out evaluation with the protocol and trial seeds of
           results/s13_imitation_static.csv: true Gittins, empirical Gittins, FCFS, rank by
           sample mean excess, the three imitation nets, and every label-free run, paired;
           Spearman agreement with the Gittins rank ordering.
...
def log_hparams(row: dict):
    """Append one row per optimizer run to the hyperparameter log."""
...
def training_set(n_train: int, seed: int, h: float = 0.01):
    """Random distributions from the imitation generators (held-out parameters excluded there),
    each with one window of N_SAMPLES. Distributions with fewer than 5 distinct sample values
    or CV^2 below 0.05 are skipped."""
...
def mean_excess_policy(sizes_u, L):
    """Rank = conditional mean excess E[S - a | S > a] of the sample."""
