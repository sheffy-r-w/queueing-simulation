...
from egittins.plotting import use_style, savefig, BLUE, ORANGE, AQUA, INK, INK2
...
def main(seed: int = 7, n: int = 40, a_u: int = 50):
    ...
    ax1.scatter(Fx, U, s=16, color=INK2, zorder=2, label="atoms  (F(x_j), E[S ∧ x_j])")
    ...
    ax1.scatter([xq], [yq], s=60, color=ORANGE, zorder=5, marker="D",
                label=f"query  (F(a), E[S ∧ a]),  a = {a_u * G.h:g}")
    ...
    ax1.set_ylabel("E[S ∧ x]  expected work up to x")
    ...
    ax2.plot(emp.K, emp.direct_ms, "o-", color=BLUE, label="direct, O(K · L)")
    ax2.plot(emp.K, emp.hull_ms, "s-", color=ORANGE, label="convex hull, O(K + L)")
    for _, row in df[~df.distribution.str.startswith("empirical")].iterrows():
        ax2.scatter([row.K], [row.direct_ms], color=BLUE, marker="*", s=90, zorder=5)
        ax2.scatter([row.K], [row.hull_ms], color=ORANGE, marker="*", s=90, zorder=5)
    ...
    ax2.set_xscale("log")
    ax2.set_yscale("log")
    ax2.set_xlabel("number of atoms K   (empirical bounded Pareto; ★ = true distributions)")
